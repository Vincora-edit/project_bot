...
async def generate_digest(
        ...
        period: str = "за период"
    ) -> str:
        """
        Генерирует дайджест по переписке с клиентом.

        Args:
            messages: Список сообщений [{from_name, text, is_project, timestamp}]
            client_info: Информация о клиенте из базы знаний (опционально)
            period: Описание периода (например "за неделю")

        Returns:
            str: Структурированный дайджест
        """
        if not messages:
            return "📭 За указанный период сообщений не найдено."
        ...
        conversation_text = "\n".join(conversation[-100:])  # Последние 100 сообщений
        ...
        if client_info:
            ...
            if client_info.get("client_name"):
                fields.append(f"Клиент: {client_info['client_name']}")
            if client_info.get("decision_maker"):
                fields.append(f"ЛПР: {client_info['decision_maker']}")
            if client_info.get("service_type"):
                fields.append(f"Услуга: {client_info['service_type']}")
            if client_info.get("preferences"):
                fields.append(f"Любит: {client_info['preferences']}")
            if client_info.get("dislikes"):
                fields.append(f"Не любит: {client_info['dislikes']}")
            if fields:
                client_context = "## Информация о клиенте\n" + "\n".join(fields) + "\n\n"

        system_prompt = f"""Ты аналитик маркетингового агентства. Составь краткий дайджест переписки с клиентом {period}.

{client_context}## Формат дайджеста
Ответ СТРОГО в таком формате:

📊 СТАТУС: [Общее настроение клиента: доволен/нейтрален/есть вопросы/недоволен]
...
⚠️ ПРОБЛЕМЫ/ЗАПРОСЫ:
• [если были — перечисли, если нет — "Нет открытых проблем"]

✅ ВЫПОЛНЕНО:
• [что было сделано командой]

📋 НУЖНО СДЕЛАТЬ:
• [что осталось сделать или на что обратить внимание]

💡 РЕКОМЕНДАЦИИ:
• [1-2 совета для проджекта]

## Правила
- Пиши кратко, по делу
- Выделяй главное
- Если информации мало — пиши "Недостаточно данных"
- Не выдумывай то, чего нет в переписке"""

        result = self._call_gpt(system_prompt, conversation_text, max_tokens=800, temperature=0.5)

        if not result:
            result = "❌ Не удалось сгенерировать дайджест. Попробуйте позже."
